    ...
    , ui(new Ui::clamav_gui)
{
    ...
    if (tempDir.exists(
        QDir::homePath() + "/.kde4/share/kde4/services/ServiceMenus/scanWithClamAV-GUI.desktop") == false)
    {
        //creating service Menu
        if (tempDir.exists(QDir::homePath() + "/.kde4/share/kde4/services/ServiceMenus"))
        {
            tempDir.mkdir(QDir::homePath() + "/.kde4/share/kde4/services/ServiceMenus");
        }
        setupFileHandler* serviceFile = new setupFileHandler(
            QDir::homePath() + "/.kde4/share/kde4/services/ServiceMenus/scanWithClamAV-GUI.desktop");
    ...
    if (tempDir.exists(QDir::homePath() + "/.clamav-gui/quarantine") == false)
    {
        tempDir.mkdir(QDir::homePath() + "/.clamav-gui/quarantine");
    }
    if (tempDir.exists(QDir::homePath() + "/.clamav-gui/signatures") == false)
    {
        tempDir.mkdir(QDir::homePath() + "/.clamav-gui/signatures");
    }
    if (tempDir.exists(QDir::homePath() + "/.clamav-gui/profiles") == false)
    {
        tempDir.mkdir(QDir::homePath() + "/.clamav-gui/profiles");
    }
    if (tempDir.exists(QDir::homePath() + "/.clamav-gui/logs") == false)
    {
        tempDir.mkdir(QDir::homePath() + "/.clamav-gui/logs");
    }
    if (tempDir.exists(path) == false)
    {
        ...
        {
            setupFile->setSectionValue("Directories", "LoadSupportedDBFiles",
                                       "not checked|" + QDir::homePath() + "/.clamav-gui/signatures");
        }
        else
        {
            setupFile->setSectionValue("Directories", "LoadSupportedDBFiles",
                                       "checked|" + QDir::homePath() + "/.clamav-gui/signatures");
        }
        setupFile->setSectionValue("Directories", "TmpFile", "checked|/tmp");
        setupFile->setSectionValue("Directories", "ScanReportToFile",
                                   "not checked|" + QDir::homePath() + "/.clamav-gui/logs/default.log");
        setupFile->setSectionValue("Directories", "MoveInfectedFiles",
                                   "not checked|" + QDir::homePath() + "/.clamav-gui/quarantine");
        setupFile->setSectionValue("Directories", "CopyInfectedFiles",
                                   "not checked|" + QDir::homePath() + "/.clamav-gui/quarantine");
    }
    else
    {
    ...
    connect(scanProcess, SIGNAL(finished(int, QProcess::ExitStatus)), this,
            SLOT(slot_scanProcessFinished(int, QProcess::ExitStatus)));
    ...
    ui->tabWidget->addTab(scannerTab, QIcon(":/icons/Clam.png"), tr("Scan"));
    optionTab = new optionsDialog(this);
    ui->tabWidget->addTab(optionTab, QIcon(":/icons/options.png"), tr("Options"));
    profileManagerTab = new ProfileManager(this);
    ui->tabWidget->addTab(profileManagerTab, QIcon(":/icons/profilemanager.png"), tr("Profile Manager"));
    freshclamTab = new freshclamsetter(this);
    ui->tabWidget->addTab(freshclamTab, QIcon(":/icons/freshclam.png"), tr("FreshClam"));
    schedulerTab = new scheduler(this);
    ui->tabWidget->addTab(schedulerTab, QIcon(":/icons/scheduler.png"), tr("Scheduler"));
    logTab = new logViewerObject(this);
    ui->tabWidget->addTab(logTab, QIcon(":/icons/includeexclude.png"), tr("Logs"));
    infoTab = new infoDialog(this);
    ui->tabWidget->addTab(infoTab, QIcon(":/icons/information.png"), tr("Credits && Copyright"));
    ui->tabWidget->setTabShape(QTabWidget::Triangular);
    connect(freshclamTab, SIGNAL(dataBaseDirectoryChanged()), optionTab, SLOT(slot_updateDirectories()));
    connect(freshclamTab, SIGNAL(setBallonMessage(int, QString, QString)), this,
            SLOT(slot_setTrayIconBalloonMessage(int, QString, QString)));
    ...
    connect(schedulerTab, SIGNAL(triggerScanJob(QString, QStringList)), this,
            SLOT(slot_receiveScanJob(QString, QStringList)));
...
void clamav_gui::slot_scanRequest(QStringList scanObjects)
{
    ...
    {
        setupFile->getSectionBoolValue("REGEXP_and_IncludeExclude", "SkipPUAPacked") == true
        ? parameters << "--exclude-pua=Packed"
        : parameters << "--include-pua=Packed";
        setupFile->getSectionBoolValue("REGEXP_and_IncludeExclude", "SkipPUAPWTool") == true
        ? parameters << "--exclude-pua=PWTool"
        : parameters << "--include-pua=PWTool";
        setupFile->getSectionBoolValue("REGEXP_and_IncludeExclude", "SkipPUANetTool") == true
        ? parameters << "--exclude-pua=NetTool"
        : parameters << "--include-pua=NetTool";
        setupFile->getSectionBoolValue("REGEXP_and_IncludeExclude", "SkipPUAP2P") == true
        ? parameters << "--exclude-pua=P2P"
        : parameters << "--include-pua=P2P";
        setupFile->getSectionBoolValue("REGEXP_and_IncludeExclude", "SkipPUAIRC") == true
        ? parameters << "--exclude-pua=IRC"
        : parameters << "--include-pua=IRC";
        setupFile->getSectionBoolValue("REGEXP_and_IncludeExclude", "SkipPUARAT") == true
        ? parameters << "--exclude-pua=RAT"
        : parameters << "--include-pua=RAT";
        setupFile->getSectionBoolValue("REGEXP_and_IncludeExclude", "SkipPUANetToolSpy") == true
        ? parameters << "--exclude-pua=NetToolSpy"
        : parameters << "--include-pua=NetToolSpy";
        setupFile->getSectionBoolValue("REGEXP_and_IncludeExclude", "SkipPUAServer") == true
        ? parameters << "--exclude-pua=Server"
        : parameters << "--include-pua=Server";
        setupFile->getSectionBoolValue("REGEXP_and_IncludeExclude", "SkipPUAScript") == true
        ? parameters << "--exclude-pua=Script"
...
void clamav_gui::slot_scanProcessFinished(int exitCode, QProcess::ExitStatus status)
{
    ...
    else
    {
        if (exitCode == 0)
        {
            ...
            trayIcon->showMessage(tr("Scan-Status"), tr("Scan Process finished ..... no virus found!"),
                                  QSystemTrayIcon::Information, 5000);
        }
        else if (exitCode == 1)
        {
            scannerTab->setStatusBarMessage(tr("Scan-Process finished ...... Virus found!"), "#ff0000");
            trayIcon->showMessage(tr("Scan-Status"), tr("Scan Process finished ..... a virus was found!"),
                                  QSystemTrayIcon::Critical, 5000);
        }
        else
        {
            scannerTab->setStatusBarMessage(tr("Scan-Process finished ...... an error occured!"), "#ff0000");
            trayIcon->showMessage(tr("Scan-Status"), tr("Scan Process finished ..... an error occurred!"),
...
void clamav_gui::slot_receiveScanJob(QString name, QStringList parameters)
{
    ...
    connect(scanObject, SIGNAL(sendStatusReport(int, QString, QString)), this,
            SLOT(slot_setTrayIconBalloonMessage(int, QString, QString)));
