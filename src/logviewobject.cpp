...
logViewObject::logViewObject(QWidget* parent, QString fileName)
    : QDialog(parent)
    , ui(new Ui::logViewObject)
{
    setWindowFlags(
...
void logViewObject::slot_closeButtonClicked()
{
    ...
    if (contentChanged)
    {
        if (QMessageBox::information(this, tr("INFO"), tr("Log-File was modified. Do you wanna save the changes?"),
...
void logViewObject::slot_clearLogButtonClicked()
{
    ...
    if (currentTab > -1)
    {
        if (QMessageBox::question(this, tr("Clear Log"), tr("Do you realy want to remove this partial log?"),
                                  QMessageBox::Yes, QMessageBox::No) == QMessageBox::Yes)
        {
