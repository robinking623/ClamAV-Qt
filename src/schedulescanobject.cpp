...
scheduleScanObject::scheduleScanObject(QWidget* parent, QString name, QStringList parameters)
    : QDialog(parent)
    , ui(new Ui::scheduleScanObject)
    , scanJob(name)
{
    setWindowFlags(
        ((this->windowFlags() | Qt::CustomizeWindowHint) & ~Qt::WindowCloseButtonHint & ~Qt::WindowContextHelpButtonHint));
    ui->setupUi(this);
    if (name == "Direct Scan")
    {
        directScan = true;
...
{
    if (directScan == false) emit { closeRequest(this); }
    else
    { this->done(0); }
}
...
void scheduleScanObject::slot_totalErrorButtonClicked()
{
    ...
    if (pos > -1)
    {
        ...
        ui->logMessagePlainTextEdit->ensureCursorVisible();
        if (ui->logMessagePlainTextEdit->toPlainText().indexOf(searchString, errorStart) == -1)
        { errorStart = 0; }
    }
}
...
void scheduleScanObject::slot_infectedFilesButtonClicked()
{
    ...
    if (pos > -1)
    {
        ...
        ui->logMessagePlainTextEdit->setTextCursor(cursor);
        ui->logMessagePlainTextEdit->ensureCursorVisible();
        if (ui->logMessagePlainTextEdit
                ->toPlainText()
                .toUpper()
                .indexOf(searchString, infectedStart) == -1)
        {
            infectedStart = 0;
        }
    }
