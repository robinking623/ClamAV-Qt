    ...
    , ui(new Ui::scheduler)
{
    ...
    ui->setupUi(this);
    header << tr("ID") << tr("Interval") << tr("Profile") << tr("Last Scan") << tr("Next Scan") << tr("Remove") << tr(
        "Scan Now") << tr("Log-File");
...
void scheduler::slot_addDailyScanJobButtonClicked()
{
    ...
    entry = "daily|" + ui->profileComboBox->currentText() + "|" + "never" + "|" + QString::number(
        nextScanObject.toMSecsSinceEpoch());
...
void scheduler::slot_addWeeklyScanJobButtonClicked()
{
    ...
    entry = "weekly|" + ui->profileComboBox->currentText() + "|" + "never" + "|" + QString::number(
        nextScanObject.toMSecsSinceEpoch());
    setupFile->setSectionValue("ScanJobs", id, entry);
    updateScheduleList();
}
...
void scheduler::slot_addMonthlyScanJobButtonClicked()
{
    ...
    if (scanTime.length() < 8)
    { scanTime = scanTime + ":00"; }
    QDateTime nextScanObject = QDateTime(
        QDate(QDate::currentDate().year(), QDate::currentDate().month(), ui->monthlyDaySpinBox->value()),
        QTime::fromString(scanTime));

    if (nextScanObject < QDateTime::currentDateTime())
    {
        nextScanObject = nextScanObject.addMonths(1);
    }
    entry = "monthly|" + ui->profileComboBox->currentText() + "|" + "never" + "|" + QString::number(
        nextScanObject.toMSecsSinceEpoch());
    setupFile->setSectionValue("ScanJobs", id, entry);
    updateScheduleList();
}
    ...
    while (ui->scanJobTableWidget->rowCount() > 0)
    {
        ...
        foreach(QString job, jobs)
        {
            ...
            else
            {
                ...
                ui->scanJobTableWidget
                    ->setItem(rowCount, 3, new QTableWidgetItem(tempDateTime.toString("dd.MM.yyyy 'at' hh:mm")));
            }
            tempDateTime.setMSecsSinceEpoch(jobData[3].toLongLong());
            ui->scanJobTableWidget
                ->setItem(rowCount, 4, new QTableWidgetItem(tempDateTime.toString("dd.MM.yyyy 'at' hh:mm")));
...
{
    int rc = QMessageBox::information(this, tr("Start Scan-Job"), tr("Do you realy want to start this Scan-Job?"),
...
{
    int rc = QMessageBox::information(this, tr("Remove Entry"), tr("Do you realy want to remove this entry?"),
                                      QMessageBox::Yes, QMessageBox::No);
    QString jobID;
...
{
    setupFileHandler* setupFile = new setupFileHandler(
    ...
    triggerScanJob(profileName, parameters);
}
    ...
    QDateTime scanDateTime;
        ...
        foreach(QString scanJob, scanJobs)
        {
            ...
            if (scanDate < today)
            {
                ...
                line = values[0] + "|" + values[1] + "|" + QString::number(today) + "|" + QString::number(
                    scanDateTime.toMSecsSinceEpoch());
...
void scheduler::slot_profileSelectionChanged()
{
    ...
    setupFileHandler* tempSetupFile = new setupFileHandler(
        QDir::homePath() + "/.clamav-gui/profiles/" + profileName + ".ini");
    QStringList targets;
    QString targetLabel;
    QStringList options;
