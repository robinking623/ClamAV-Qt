...
scanTab::scanTab(QWidget* parent)
    : QWidget(parent)
    , ui(new Ui::scanTab)
{
    ui->setupUi(this);
    ...
    setupFile->getSectionBoolValue("Settings", "RecursivScan") == true
    ? ui->recursivCheckBox->setChecked(true)
    : ui->recursivCheckBox->setChecked(false);
...
void scanTab::slot_scanButtonClicked()
{
    ...
    {
        if (i->data(QFileSystemModel::FilePathRole).toString() != "")
        {
            scanObjects << i->data(QFileSystemModel::FilePathRole).toString();
        }
    }
    ...
    if (dirs.count() > 0)
    {
        ...
        buttonID = 0;
            foreach (QString entry, dirs)
            {
                ...
                dragablePushButton* button = new dragablePushButton(QIcon(":/icons/icons/media.png"),
                                                                    entry.mid(entry.lastIndexOf("/") + 1), this,
                                                                    (QString) ("/var/run/media/" + username.trimmed() + "/" + entry));
