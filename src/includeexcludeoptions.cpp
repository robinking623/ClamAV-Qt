...
includeExcludeOptions::includeExcludeOptions(QWidget* parent)
    : QWidget(parent)
    , ui(new Ui::includeExcludeOptions)
{
    ui->setupUi(this);
...
void includeExcludeOptions::readSettings()
{
    ...
    checked == "checked" ? ui->dontScanFileNameCheckBox->setChecked(true) : ui->dontScanFileNameCheckBox
        ->setChecked(false);
    ...
    checked = value.left(value.indexOf("|"));
    value = value.mid(value.indexOf("|") + 1);
    checked == "checked" ? ui->onlyScanFileNameCheckBox->setChecked(true) : ui->onlyScanFileNameCheckBox
        ->setChecked(false);
