...
partialLogObject::partialLogObject(QWidget* parent, QString logText)
    : QWidget(parent)
    , ui(new Ui::partialLogObject)
{
    ui->setupUi(this);
...
void partialLogObject::slot_totalErrorButtonClicked()
{
    ...
    if (pos > -1)
    {
        ...
        ui->logPlainText->setTextCursor(cursor);
        ui->logPlainText->ensureCursorVisible();
        if (ui->logPlainText->toPlainText().indexOf(searchString, errorStart) == -1)
        { errorStart = 0; }
    }
}
...
void partialLogObject::slot_infectedFilesButtonClicked()
{
    ...
    int pos = ui->logPlainText->toPlainText().toUpper().indexOf(searchString, infectedStart);
    ...
    if (pos > -1)
    {
        ...
        ui->logPlainText->setTextCursor(cursor);
        ui->logPlainText->ensureCursorVisible();
        if (ui->logPlainText->toPlainText().toUpper().indexOf(searchString, infectedStart) == -1)
        { infectedStart = 0; }
    }
}
