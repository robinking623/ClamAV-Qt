...
void QRoundProgressBar::drawBase(QPainter& p, const QRectF& baseRect)
{
    switch (m_barStyle)
    {
        ...
        case StyleLine:
            ...
            p.drawEllipse(baseRect.adjusted(m_outlinePenWidth / 2, m_outlinePenWidth / 2, -m_outlinePenWidth / 2,
                                            -m_outlinePenWidth / 2));
            break;
...
void QRoundProgressBar::drawValue(QPainter& p, const QRectF& baseRect, double value, double delta)
{
    ...
    if (m_barStyle == StyleLine)
    {
        ...
        {
            p.drawEllipse(
                baseRect.adjusted(m_outlinePenWidth / 2, m_outlinePenWidth / 2, -m_outlinePenWidth / 2,
                                  -m_outlinePenWidth / 2));
        }
        else
        {
...
void QRoundProgressBar::drawText(QPainter& p, const QRectF& innerRect, double innerRadius, double value)
{
    ...
    f.setPixelSize(10);
    QFontMetricsF fm(f);
    double maxWidth = fm.horizontalAdvance(valueToText(m_max));
